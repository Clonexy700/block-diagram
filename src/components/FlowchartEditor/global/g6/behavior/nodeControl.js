  ...
  options: {
    ...
    drawLine: {
      ...
      stop (event) {
        ...
        if (_t.drawLine.isMoving) {
          if (_t.drawLine.currentLine === event.item) {console.log('移除了',event.item)
            // 画线过程中点击则移除当前画线
    ...
    shapeControlPoint: {
      ...
      start (event) {
        ...
        // 是否等比缩放
        _t.shapeControlPoint.isProportional = ['exclusiveGateway', 'circle','start','end'].includes(model.shape)
    ...
    dragNode: {
      ...
      createNode (event) {
        let _t = this
        if (_t.dragNode.dottedNode && _t.info.node) {
          let { width, height, minWidth, minHeight,shape } = _t.info.node
          let node = {
            ..._t.info.node,
            id: shape+'_'+G6.Util.uniqueId(),
            x: event.x,
            y: event.y,
            size: [width, height],
            minWidth: minWidth,
      ...
      start (event) {
        ...
        if (_t.config.tooltip.dragNode) {
          ...
          _t.toolTip.create.call(_t, {
            left: event.canvasX,
            top: event.canvasY + height / 2
          }, `X: ${event.canvasX.toFixed(2)} Y: ${event.canvasY.toFixed(2)}<br>W: ${width.toFixed(2)} H: ${height.toFixed(2)}`)
        }
        _t.dragNode.status = 'dragNode'
        /* 点击的节点上增加鼠标位置变量做标记，end时删除此变量 */
        var startNodeModel = _t.info.node.getModel();
        startNodeModel.eventTarget = {
          estartX: event.canvasX,
          estartY: event.canvasY,
          startX: startNodeModel.x,
          startY: startNodeModel.y,
        }
        // console.log('start',event.canvasX,event.canvasY)
      },
      move (event) {
        let _t = this
        if (_t.dragNode.status === 'dragNodeToEditor') {
          ...
          if (_t.info.node) {
            ...
            let attrs = {
              x: startNodeModel.eventTarget.startX + event.canvasX - startNodeModel.eventTarget.estartX,
              y: startNodeModel.eventTarget.startY+ event.canvasY - startNodeModel.eventTarget.estartY
              // ,
              // style: {
              //   ...style,
              //   fill: _t.graph.$X.fill,
